...
from exchange_order_dispatchers.constants import (
    KRAKEN_API_VERSION,
    KRAKEN_KEY_HEADER,
    KRAKEN_SIGNATURE_HEADER,
    KRAKEN_BASE_REST_API,
    MS_PER_SECOND,
)
...
class KrakenAPI(object):
    ...
    def __init__(self, key="", secret=""):
        ...
        self.key = key
        self.secret = secret
        self.uri = KRAKEN_BASE_REST_API
        self.apiversion = KRAKEN_API_VERSION
    ...
    def _query(self, urlpath, data, headers=None, timeout=None):
        ...
        if self.response.status_code not in (
            HTTPStatus.OK.value,
    ...
    def query_private(self, method, data=None, timeout=None):
        ...
        headers = {
            KRAKEN_KEY_HEADER: self.key,
            KRAKEN_SIGNATURE_HEADER: self._sign(data, urlpath),
        }
